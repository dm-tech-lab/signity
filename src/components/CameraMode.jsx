...
function CameraMode() {
		...
		if (result.multiHandLandmarks && result.multiHandedness) {
		  ...
		  if (result.multiHandedness.length === 1) {
			...
			if (gesture.gestures[0]) {
			  ...
			  else if (sign === "Н" && sentence[sentence.length - 2] !== "Н" && sentence[sentence.length - 1] === "Н" && sign !== ".") {
				sentence.push(sign);
			  }
			  else if (sign === "Т" && sentence[sentence.length - 2] !== "Т" && sentence[sentence.length - 1] === "Т" && sign !== ".") {
				sentence.push(sign);
			  }
			...
			if (leftGesture.gestures[0] && rightGesture.gestures[0]) {
			  ...
			  if (sentence[sentence.length - 1] !== "Благодаря ви" && (signL === "В" && signR === "В")) {
	...
	//context
	const { openModal, setOpenModal,avatarImg,setAvatarImg } = useContext(UserContext)
	...
	const SignUp = () => {
		...
		return (
			<>
				 {recaptcha && <PhoneNumberVerification isOpenSignUp={isOpenSignUp} setIsOpenSignUp={setIsOpenSignUp} setAvatarImg={setAvatarImg} recaptcha={recaptcha} />}
				<div ref={element}></div>
			</>
		);
	}
	...
	<div>
		<p>
			Здравей 🖐 Направи знак пред камерата, за да започна да превеждам. Да започнем с нещо лесно, кажи “Здрасти”.
		</p>
				...
				<div className='chat'>
					<div>
						...
						<p className='messageOne'>
							{sentence.length === 0 && messages.length < 1 ? newUser : sentence}
						</p>
						<div>
							{messages.map((item, index) => (
								<div>
									<p className='message' key={item.index + 1}>{item}</p>
					...
					<div className='reset-chat'>
						<button className='underline px-6 py-6 align-bottom leading-4 delEntireRow' onClick={clearSentence}>Нулиране</button>
						<button className='underline px-6 py-6 align-bottom leading-4 
						delLastChar' onClick={deleteCharacter}>Изтрий последната буква</button>
					</div>
				</div>
			</div>
		{user ?
			<div>
				...
				<div className='absolute top-8 right-8 flex justify-center items-center chat-system'>
							{browserSupportsSpeechRecognition ? 
								<button className='underline mr-4' onClick={() => navigate('/animation')}>Включи режим Анимация</button>
								:
								<button onClick={() => browserSupportsSpeechRecognition ? null : setBrowserMode(true)} className='underline mr-4'>
				...
				{browserMode ? <BrowserSupportModal browserMode={browserMode} setBrowserMode={setBrowserMode}/> : null}

				<div className='mr-5'>
					<div className='absolute bottom-6 right-10'>
						<button className=" mr-4" onClick={() => navigate('/suggestions')}>Направи Предложение</button>
						|
						<button className="ml-4" onClick={toggleModal}>За нас</button>
					</div>
					{!openModal ? null : <AboutModal />}
				</div>
				<div className='absolute bottom-6 left-10'>
					<button className=" ml-2" onClick={() => navigate('/')}>Назад</button>
				</div>
			</div>
			:
			<div>
				<div className='absolute top-10 right-10 flex askingRegister'>
					<button className="underline mr-1" onClick={() => setIsOpenSignUp(true)}>Влез в акаунта си,</button> 
				...
				<div className='absolute bottom-6 right-10'>
					...
					{needRegisModal ? <Modal SignUp={SignUp} isOpenSignUp={isOpenSignUp} setIsOpenSignUp={setIsOpenSignUp} needRegisModal={needRegisModal} setNeedRegisModal={setNeedRegisModal}/> : null}

					<button className="mr-4" onClick={() => setNeedRegisModal(true)}>Направи Предложене</button>
					|
					<button className="ml-4" onClick={toggleModal}>За нас</button>
					{!openModal ? null : <AboutModal />}
				</div>
